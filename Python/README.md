...
1. メソッドの宣言、戻り値、Docstringを書く
   1. 型ヒントも書く
2. Docstringの中に、doctestを書く
3. ユニットテストを書く
4. ユニットテストをPassできる最低限の実装をし、ユニットテストとdoctestをパスする
   1. このとき、要件は満たさなくていい。ただテストが通るだけのハリボテを書くだけ
5. 要件を満たす最低限の実装をし、ユニットテストとdoctestをパスする
   1. コメントも書く
6. リファクタリングし、ユニットテストとdoctestをパスする
7. コードを`commit`し、`push`する
...
Docstringは、そのモジュール・関数・クラスを呼び出して使用する人が、コードを見なくても、呼び出して使用できるようにするためのものです。コードを読みやすくするためのものではないことに注意してください。なので、呼び出して使用するのに必要な最低限の説明だけあれば十分で、それ以上の実装の詳細などを記述する必要はありません。また同様の理由で、呼び出して使用することを想定していないモジュール・関数・クラス（プライベートメソッドなど）にはDocstringを記述しません。
